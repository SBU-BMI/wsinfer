...
def configure_qupath() -> None:
    try:
        ...
        choice = input(
            """QuPath can be configured by setting the 'qupath_dir' field in '.paquo.toml'.
        ...
        if choice is None or choice != "Y":
            ...
            qupath_directory = input(
                "Please enter the exact path where QuPath is installed: "
            )

            if Path(qupath_directory).exists():
                os.environ["PAQUO_QUPATH_DIR"] = str(
                    qupath_directory
                )  # setting the env var
...
def make_qupath_project(wsi_dir: Path, results_dir: Path) -> None:
    ...
    json_list = natsorted(
        [
            str(file)
            for file in Path(f"{results_dir}/model-outputs-geojson").iterdir()
            if file.is_file()
        ]
    )

    slides_and_geojsons = [(csv, json) for csv, json in zip(csv_list, json_list)]
